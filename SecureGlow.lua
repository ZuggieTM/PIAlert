...
function SecureGlow:Stop(frame)
    if not frame then return end
    pcall(StopPixel, frame)
    pcall(StopFlipbook, frame)
end

-- Pixel glow for Blizzard 12.1 CustomAuraContainer button descendants.
-- It deliberately uses C-side AnimationGroups rather than a Lua OnUpdate.
-- Once created/started in initializeFrame, it continues animating even after
-- Blizzard marks the aura-button subtree forbidden/secret. The parent aura
-- button still owns visibility, so the glow appears only while Blizzard says
-- the tracked helpful aura is present.
function SecureGlow:StartPixel(frame, width, height, color, cfg)
...
function SecureGlow:Start(frame, width, height, color, cfg)
    cfg = cfg or {}
